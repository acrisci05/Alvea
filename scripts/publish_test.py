...
def make_reading(scenario):
    ...
    if scenario == "hardware_fault":
        contact = False
        status = "ERR_PPG_NO_CONTACT"
        bpm, skin_temp, spo2, resp_rate = 0.0, 0.0, 0.0, 0.0
        
    # 2. SCENARIO: Attacco d'asma (Ipossia + Tachipnea + Tachicardia lieve)
        ...
        spo2 = round(random.uniform(88.0, 92.0), 1)          # Crollo ossigenazione
...
def main():
    ...
    # In produzione, se il broker ha una password, andrebbe aggiunta qui:
    ...
    # client.username_pw_set(args.user, args.password) # Decommentare se il broker e' protetto
    ...
    try:
        while True:
            ...
            print(f"TX: {json.dumps(payload, indent=2)}") # Stampa pretty-print nel terminale
