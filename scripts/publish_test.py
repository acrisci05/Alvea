...
def make_reading(scenario):
    ...
    if scenario == "hardware_fault":
        contact = False
        bpm, skin_temp, resp_rate, spo2 = 0.0, 0.0, 0.0, 0.0

    # 2. SCENARIO: Attacco d'asma (Tachipnea + Tachicardia lieve + SpO2 bassa)
        ...
        spo2 = round(random.uniform(90.0, 94.0), 1)   # desaturazione

    # 3. SCENARIO: Febbre
    elif scenario == "fever":
        bpm = round(random.uniform(110.0, 120.0), 1)
        skin_temp = round(random.uniform(35.5, 37.0), 1)
