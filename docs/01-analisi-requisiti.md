...
Progetto **Alvea** — dispositivo indossabile da caviglia per il monitoraggio dell'asma pediatrico (frequenza respiratoria, battito cardiaco e temperatura cutanea).
...
|----|-------------|---|-----------|---|
| RQ-01 | Stimare la frequenza respiratoria dall'ECG (EDR, Filtro IIR sul baseline wandering) | F | Sensing | 0 |
| RQ-02 | Acquisire battito cardiaco (ECG AD8232) e temperatura cutanea (termistore NTC) | F | Sensing | 1 |
| RQ-03 | Rilevare l'aderenza del dispositivo (ECG leads-off) | F | Sensing | 1 |
| RQ-04 | Pubblicare la telemetria a 1 Hz su topic MQTT `alvea/devices/{ID}/telemetry` | F | Telemetry | 0 |
| RQ-05 | Payload JSON: `{device_id, timestamp, bpm, respiration_rate, skin_temperature, sensor_contact, device_status}` | F | Telemetry | 0 |
...
| RQ-08 | Valutare soglie cliniche (tachipnea, tachicardia, febbre) e generare alert per asma | F | Processing | 0 |
...
| ID | Descrizione | T | Categoria | P |
|----|-------------|---|-----------|---|
| RQ-16 | Esecuzione asincrona non bloccante (ECG 250 Hz, EDR 25 Hz, RingBuffer statico) | NF | Performance | 0 |
