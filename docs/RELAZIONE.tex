...
\begin{enumerate}[leftmargin=1.5em,itemsep=3pt]
  ...
  \item \textbf{Livello di batteria.} Una cella LiPo a singola tensione è monitorata tramite un partitore resistivo collegato a un canale ADC1 dedicato (GPIO25), distinto e non in conflitto con i canali usati da ECG e sensore di temperatura.
\end{enumerate}
...
Oltre alla telemetria periodica, il firmware pubblica autonomamente, sul topic \texttt{alvea/devices/\{ID\}/alerts}, allarmi di natura tecnica non derivabili dalla sola analisi delle soglie cliniche lato backend: guasto persistente di un sensore (elettrodi ECG scollegati, sensore di temperatura non disponibile) e batteria scarica. Per evitare di generare un alert per ogni singolo glitch transitorio, una condizione viene segnalata solo dopo 5 letture consecutive anomale, e un secondo alert di risoluzione viene generato non appena la condizione rientra.

% =====================================================================
\section{Ruoli, Autenticazione e Multitenancy}

Il backend implementa oggi un'autenticazione a token \textbf{JWT} con hashing delle password (bcrypt) e isolamento dei dati per account: ogni \texttt{Caregiver} vede solo i device che ha registrato (\texttt{owner\_id}). Non esiste invece, allo stato attuale del repository, una distinzione di \emph{ruolo} (Role-Based Access Control) tra Medico e Paziente: l'entità persistita è un unico \texttt{Caregiver} generico, senza campo \texttt{role}, e qualunque account autenticato ha gli stessi permessi sui propri device.

La separazione Paziente/Caregiver vs Medico Pediatra descritta di seguito è l'architettura \emph{progettata} (\PLAN, vedi Sezione~\ref{sec:stato}), pensata per un'evoluzione naturale del sistema:

\begin{itemize}[leftmargin=1.3em,itemsep=2pt]
  \item \textbf{Paziente / Caregiver.} Accede all'App mobile (React Native), visualizzando \emph{esclusivamente} la telemetria e gli alert del proprio bambino. Riceve notifiche realtime via WebSocket/SSE. È l'unico ruolo oggi realmente implementato.
  \item \textbf{Medico Pediatra (progettato).} Supervisionerebbe molteplici dispositivi, consulterebbe i grafici storici avanzati su \textbf{Grafana} e potrebbe gestire la configurazione remota dei device (frequenza di campionamento, associazione paziente-dispositivo) con permessi distinti da quelli del Caregiver. Avrebbe accesso in lettura/scrittura alla \textbf{Scheda Anamnestica} del paziente (patologie, allergie, terapie cortisoniche) — entità anch'essa non ancora persistita (\PLAN).
\end{itemize}

Nel frattempo, l'app mobile espone già la dashboard Grafana e il pulsante di configurazione remota a chiunque sia autenticato (vedi \texttt{SHOW\_GRAFANA\_TAB} in \texttt{mobile/src/config.js}), in attesa che il backend distingua i due ruoli.
...
Autenticazione JWT a singolo ruolo (Caregiver) con isolamento dati per account & \texttt{backend/app/} & \IMPL \\
...
Autenticazione multi-ruolo (RBAC: Medico/Paziente) & --- & \PLAN \\
\bottomrule
\end{tabularx}
\end{table}
