...
\begin{itemize}[leftmargin=1.3em,itemsep=2pt]
  \item \textbf{Saturazione di Ossigeno (SpO2)} -- indicatore primario per valutare l'ipossia causata da broncocostrizione, calcolata dal rapporto AC/DC delle componenti Rosso/Infrarosso del segnale PPG (Ratio of Ratios).
  \item \textbf{Frequenza Respiratoria (Atti/min)} -- estratta dal canale infrarosso del segnale fotopletismografico (PPG) tramite un filtro IIR passa-basso che isola la componente respiratoria lenta dalla pulsazione cardiaca sovrapposta, per identificare tachipnea da sforzo respiratorio.
...
\begin{enumerate}[leftmargin=1.5em,itemsep=3pt]
  \item \textbf{PPG per SpO2 e Respiro (PPG analogico, due canali).} Front-end ottico realizzato in elettronica discreta (fotodiodo + amplificatore a transimpedenza), campionato su due canali ADC1 indipendenti dell'ESP32: \textbf{GPIO36} per il canale rosso e \textbf{GPIO39} per il canale infrarosso, a risoluzione 12 bit e frequenza di campionamento di \textbf{50 Hz}. La SpO2 è calcolata tramite il rapporto AC/DC delle due lunghezze d'onda (Ratio of Ratios). La Frequenza Respiratoria è estratta purificando il solo canale infrarosso tramite un \textbf{Filtro IIR Passa-Basso} ($\alpha = 0.05$) che sopprime la frequenza cardiaca ed evidenzia l'onda respiratoria, analizzata poi su una finestra circolare di 10 secondi.
  \item \textbf{Frequenza Cardiaca (AD8232).} Front-end analogico ECG campionato a \textbf{250 Hz (GPIO34)}. Il BPM è ricavato mediante un algoritmo Pan-Tompkins ottimizzato con \textbf{Ring Buffer statici} per evitare allocazioni dinamiche (zero Garbage Collector) e garantire stabilità in produzione. I pin GPIO32 (LO+) e GPIO33 (LO--) forniscono il rilevamento leads-off.
...
\midrule
Front-end PPG -- canale rosso & ADC1 -- GPIO36 (12 bit, 50 Hz) & Tensione analogica (onda PPG, rosso) \\
Front-end PPG -- canale infrarosso & ADC1 -- GPIO39 (12 bit, 50 Hz) & Tensione analogica (onda PPG, IR) \\
...
\midrule
Filtro IIR per estrazione frequenza respiratoria e SpO2 (PPG) & \texttt{firmware/sensor\_ppg.py} & \IMPL \\
